...
const Register = () => {
  ...
  const [currentStep, setCurrentStep] = useState<'username' | 'pattern' | 'complete'>('username');
  const [regData, setRegData] = useState<UserRegData>({
    username: '',
    pattern: []
  });
  ...
  const simulateBotTyping = async (duration: number = 1500) => {
    setIsTyping(true);
    addMessage({
      sender: 'bot',
      text: '...',
  ...
  const handleFinalRegistration = async (pattern: string[]) => {
    setLoading(true);
    try {
